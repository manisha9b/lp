...
alert($("#cod").is(':checked'));
	...
	function codForm(div_no){		var myData = $("#DetailForm").serialize();		alert(myData);		jQuery.ajax({			type: "POST",			url: "otp.php",			 dataType: "json",			data:myData,			success:function(response){				if(response.uid!='')				{				$("#DetailForm_div"+div_no).hide();				$("#cod_div"+div_no).show();				$("#uid").val(response.uid);				}			},		});		}		function validateOtp(div_no){					var myData = $("#cod_form"+div_no).serialize();		alert(myData);		jQuery.ajax({			type: "POST",			url: "otp.php",			 dataType: "json",			data:myData,			success:function(response){				if(response.verified=='1')				{					window.location= 'thankyou.php';				}else{					$("#msg"+div_no).html('<div class="alert alert-msg alert-dismissible alert-info" role="alert">' + response.msg + '<button type="button" class="close" data-dismiss="alert" aria-label="Close"><i class="fa fa-times"></i></button></div>');				}			},		});		}
function validate1()
